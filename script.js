...
const expeditionModal=$("#expeditionModal"),expeditionStats=$("#expeditionStats"),expeditionHighlights=$("#expeditionHighlights"),expeditionScore=$("#expeditionScore"),expeditionRank=$("#expeditionRank"),expeditionRankLevel=$("#expeditionRankLevel");
...
const expeditionTitles=[
  {score:0,title:"Burtu Burtnieks"},{score:200,title:"Vārdu Vācējs"},{score:500,title:"Morfēmu Mednieks"},{score:1000,title:"Leksēmu Lietpratējs"},{score:1800,title:"Derivāciju Detektīvs"},
  {score:3000,title:"Semantikas Stratēģis"},{score:5000,title:"Vārddarināšanas Virtuozs"},{score:8000,title:"Tēzaura Takuzinis"},{score:12000,title:"Leksikona Lielmeistars"},{score:18000,title:"Valodas Visuma Valdnieks"}
];
function currentExpeditionTitle(){let level=0;for(let index=0;index<expeditionTitles.length;index++){if(totalScore<expeditionTitles[index].score)break;level=index}return{...expeditionTitles[level],level:level+1}}
function expeditionSummary(){const dialects=found.filter(word=>words[word]?.apvidvards).length,historic=found.filter(word=>words[word]?.senvards).length,longest=[...found].sort((a,b)=>[...b].length-[...a].length)[0],rank=currentExpeditionTitle();return{rank,stats:[[found.length,"ATRASTIE VĀRDI"],[unlockedBlocks.size,"ATRASTIE KLUCĪŠI"],[dialects,"APVIDVĀRDI"],[historic,"SENVĀRDI"],[missionCompletedTotal,"PABEIGTĀS MISIJAS"],[totalScore,"PUNKTI"]],highlights:[["GARĀKAIS VĀRDS",longest?`${longest} · ${[...longest].length} burti`:"—"],["EKSPEDĪCIJAS LAIKS",formatDuration(Date.now()-expeditionStartedAt)]]}}
function openExpeditionSummary(){const summary=expeditionSummary();expeditionSummaryOpen=true;graph.selected=null;expeditionScore.textContent=totalScore;expeditionRank.textContent=summary.rank.title;expeditionRankLevel.textContent=`${summary.rank.level} / ${expeditionTitles.length}`;expeditionStats.innerHTML=summary.stats.map(([value,label])=>`<div class="expedition-stat"><b>${value}</b><span>${label}</span></div>`).join("");expeditionHighlights.innerHTML=summary.highlights.map(([label,value])=>`<div class="expedition-highlight"><span>${label}</span><b>${value}</b></div>`).join("");expeditionModal.hidden=false}
...
Object.assign(graph,{
  ...
  rebuildLinks(){
    ...
    for(let i=0;i<names.length;i++)for(let j=i+1;j<names.length;j++){
      ...
      if(relation)candidates.push(relation);else if(isDirectBuild(names[i],names[j]))candidates.push({from:names[i],to:names[j],kind:"block",score:20});else if([...names[i]][0]===[...names[j]][0])candidates.push({from:names[i],to:names[j],kind:"initial",score:Math.max(5,10-Math.abs([...names[i]].length-[...names[j]].length)*.5)});
    }
